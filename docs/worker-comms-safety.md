...
## Acceptable route for sends: the raw OpenPhone/Quo API, with attribution stamped

This fallback covers **sends only** — reads have no raw fallback (see *Reading replies*).
...
the raw API has none of the broker's safety rails — no opt-out suppression, no
blocked-number handling, no dedup, no geo-gating, and no Traba-side audit record (the
only trace lives in the vendor console) — so you carry what you can yourself:
...
handling, dedup, geo-gating, and audit logging on top. The raw path with a stamped `userId` preserves
