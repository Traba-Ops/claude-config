...
Claude first checks the public API (BigQuery datasets via traba-auth, Traba MCP, documented REST endpoints) for an existing path to the data. Only if none exists does Claude drive a Chrome instance via the `chrome-devtools` MCP (`navigate_page`, `list_network_requests`, `get_network_request`, etc.) to capture the real GraphQL/REST call that loads the view — installing the MCP automatically if it is missing (`claude mcp add chrome-devtools --scope user -- npx chrome-devtools-mcp@latest`) and pausing for a session restart. Extracts the filters, plays them back to the operator in plain English for confirmation, and reconstructs the query against the sanctioned public API. Enforces row-count reconciliation before shipping and prohibits hitting the inspected custom endpoint directly from the operator's app. Includes a manual fallback for cases where the MCP can't be installed.
