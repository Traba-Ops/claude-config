...
[^3]: **What deployment means.** When your app runs on `localhost`, that literally means "this computer" — nobody else can see it. Deploying copies your app to a server in the cloud (we use Railway) that runs it 24/7 and gives it a URL. Every deployed app ships with a Google login page wired into the app itself — only people with a `@traba.work` account can get past it.
